...
class MainlandSubclusterOnlyOutput(NamedTuple):
    ...
    sample_ids_fid_iid: pd.DataFrame
    sample_id_file: Path
...
class MainlandSubclusterOnlyConfig:
    output_dir: str = "results/06_mainland_subcluster_only"
    sample_id_file: str = "mainland_subcluster_samples.fid_iid.txt"
    figure_file: str = "mainland_subcluster_only.png"
    ...
    mainland_group_color: str = "#E67E22"
...
def _resolve_assigned_group_col(df: pd.DataFrame, preferred: str) -> str:
    ...
    for candidate in ("Assigned_Mainland_Subcluster_Group", "Assigned_Customize_Group", "Assigned_Composite_Group"):
        if candidate in df.columns:
            return candidate
    raise KeyError("No assigned group column found in df_results_step5.")


def run_mainland_subcluster_only(
    *,
    df_results_step5: pd.DataFrame,
    our_case_iids: list[Any],
    our_ctrl_iids: list[Any],
    config: MainlandSubclusterOnlyConfig | None = None,
    bbj_samples_gmm: pd.DataFrame | None = None,
    eigenval: pd.DataFrame | None = None,
) -> MainlandSubclusterOnlyOutput:
    config = config or MainlandSubclusterOnlyConfig()

    if "IID" not in df_results_step5.columns:
        raise KeyError("df_results_step5 must contain IID column.")

    assigned_group_col = _resolve_assigned_group_col(df_results_step5, config.assigned_group_col)
    pc_cols = _resolve_pc_columns(df_results_step5)
    confidence_col = str(config.confidence_col)
    if confidence_col not in df_results_step5.columns:
        raise KeyError(f"df_results_step5 must contain {confidence_col}.")

    mainland_label = str(config.mainland_group_label)
    mask = df_results_step5[assigned_group_col].astype(str) == mainland_label
    df_mainland_subcluster = df_results_step5.loc[mask].copy()

    out_dir = Path(str(config.output_dir))
    out_dir.mkdir(parents=True, exist_ok=True)

    sample_id_file = out_dir / str(config.sample_id_file)
    iid_str = df_mainland_subcluster["IID"].astype(str)
    sample_ids_fid_iid = pd.DataFrame({"FID": iid_str.to_numpy(), "IID": iid_str.to_numpy()})
    sample_ids_fid_iid.to_csv(sample_id_file, sep="\t", index=False, header=False)
    ...
    df_mainland_subcluster["Group"] = group

    counts = (
        df_mainland_subcluster["Group"]
        .astype(str)
        .value_counts()
    ...
    n_unlabeled = int((df_mainland_subcluster["Group"].astype(str) == "").sum())

    confidence = pd.to_numeric(df_mainland_subcluster[confidence_col], errors="coerce").to_numpy(dtype=np.float32, copy=False)
    if not np.isfinite(confidence).all():
        raise ValueError("Selected mainland_subcluster subset contains invalid assignment confidence values.")

    our_pc1 = pd.to_numeric(df_mainland_subcluster[pc_cols[0]], errors="coerce").to_numpy(dtype=np.float64, copy=False)
    our_pc2 = pd.to_numeric(df_mainland_subcluster[pc_cols[1]], errors="coerce").to_numpy(dtype=np.float64, copy=False)
    ...
    if bool(config.save_plot) or bool(config.show_plot):
        ...
        bbj_pc1 = np.array([], dtype=np.float64)
        bbj_pc2 = np.array([], dtype=np.float64)
        if bbj_samples_gmm is not None and pc_cols[0] in bbj_samples_gmm.columns and pc_cols[1] in bbj_samples_gmm.columns:
        ...
        all_x = np.concatenate([bbj_pc1, our_pc1]) if bbj_pc1.size > 0 else our_pc1
        all_y = np.concatenate([bbj_pc2, our_pc2]) if bbj_pc2.size > 0 else our_pc2
        ...
        is_case = (df_mainland_subcluster["Group"].astype(str).to_numpy() == str(config.case_label))
        is_ctrl = (df_mainland_subcluster["Group"].astype(str).to_numpy() == str(config.control_label))
        ...
        sc = ax3.scatter(our_pc1[sort_idx], our_pc2[sort_idx], c=confidence[sort_idx], cmap="cividis", vmin=vmin, vmax=vmax, s=s_our, alpha=0.90, edgecolors="none", zorder=2)
        ...
        fig.suptitle(
            "Mainland Subcluster-Only View Under Step5 Recomputed Posterior",
            fontsize=34,
            fontweight="bold",
            y=0.965,
        )
    ...
    if bool(config.verbose):
        print("\n" + "=" * 80)
        print("STEP6: MAINLAND SUBCLUSTER ONLY".center(80))
        print("=" * 80)
        print(f"  assigned_group_col : {assigned_group_col}")
        print(f"  mainland_label     : {mainland_label}")
        print(f"  rows (unfiltered)  : {int(df_mainland_subcluster.shape[0])}")
        print(f"  unlabeled rows     : {n_unlabeled}")
        print(f"  sample_id_file     : {sample_id_file}")
        if figure_path is not None:
            print(f"  figure_file        : {figure_path}")
        print("=" * 80)

    return MainlandSubclusterOnlyOutput(
        df_mainland_subcluster=df_mainland_subcluster,
        sample_ids_fid_iid=sample_ids_fid_iid,
