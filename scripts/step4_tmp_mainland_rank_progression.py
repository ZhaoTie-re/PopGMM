...
class Step4TmpMainlandRankProgressionOutput(NamedTuple):
    """Output container for STEP4_tmp mainland rank progression analysis."""
...
class Step4TmpMainlandRankProgressionConfig:
    """Configuration for STEP4_tmp mainland rank progression analysis."""

    output_dir: str = "results/04_our_assignment/STEP4_tmp"
    rank_table_file: str = "mainland_cluster_rank_table.tsv"
    cumulative_table_file: str = "mainland_rank_cumulative_metrics.tsv"
    decision_table_file: str = "mainland_rank_decision_table.tsv"
...
def _extract_mainland_clusters(merge_map: pd.DataFrame) -> list[int]:
...
def _gwas_neff(case_n: int, ctrl_n: int) -> float:
    if case_n <= 0 or ctrl_n <= 0:
        return float("nan")
    return float(4.0 / ((1.0 / float(case_n)) + (1.0 / float(ctrl_n))))


def _overall_heterogeneity_pc12(all_xy: np.ndarray) -> float:
    """Overall heterogeneity for all included samples on PC1/PC2.

    Uses root generalized variance (RGV): H = det(Sigma)^(1/4) for 2D.
    This captures both variance magnitude and covariance structure.
    """
    if all_xy.ndim != 2 or all_xy.shape[1] != 2:
        return float("nan")
    if int(all_xy.shape[0]) < 2:
        return float("nan")

    cov = np.cov(all_xy, rowvar=False)
    cov = np.asarray(cov, dtype=np.float64)
    if not np.all(np.isfinite(cov)):
        return float("nan")

    # Regularize covariance slightly for numerical stability.
    scale = float(np.trace(cov) / 2.0) if np.isfinite(float(np.trace(cov))) else 1.0
    eps = 1e-8 * max(1.0, scale)
    cov_reg = cov + (np.eye(2, dtype=np.float64) * eps)

    det_val = float(np.linalg.det(cov_reg))
    if not np.isfinite(det_val) or det_val <= 0.0:
        return float("nan")
    return float(det_val ** 0.25)
...
def run_step4_tmp_mainland_rank_progression(
    *,
    ...
    mainland_cluster_ids = _extract_mainland_clusters(merge_map)
    if len(mainland_cluster_ids) == 0:
    ...
    all_cluster_ids = sorted(
        ...
        | set(int(x) for x in mainland_cluster_ids)
    )
    ...
    ranked_mainland_clusters = sorted(
        [int(cid) for cid in mainland_cluster_ids if int(cid) in ratio_map],
    ...
    for rank, cid in enumerate(ranked_mainland_clusters[:max_rank], start=1):
    ...
    return Step4TmpMainlandRankProgressionOutput(
